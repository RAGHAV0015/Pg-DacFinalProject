...
export default function Booknow() {
  ...
  const loadShowBookings=()=>{
    axios
    .get(baseUrl+apiUrls.CHECK_AREA_BOOKING_URL+'?showid=' + id+'&date='+showdate)
    .then((resp) => 
    {
  ...
  useEffect(() => {
    axios
      .get(baseUrl+apiUrls.AREA_URL + id)
      ...
      <div className='container mt-5'>
        ...
        <div className='row'>
          <div class='col-sm-3'>
            <div className='card'>
              <img
                src={'http://localhost:8080/' + show?.park.poster}
                className='card-img-top'
              />
              <div className='card-body text-center'>
                <h6>{show?.park.parkName} ({show?.park.year})</h6>
                <h6>{show?.park.description}</h6>
                <h6>Park owner name: {show?.park.ownerName}</h6>
                <h6>Park worker name: {show?.park.workerName}</h6>
                <h6>Park facility: {show?.park.facility}</h6><hr/>
